...
const ReduceCourseLoadForm = ({ formData: propFormData, mode = "edit" }) => {
	...
	const handleSubmit = async (status) => {
		...
		// Create a clean payload for the backend excluding user
		const updatedFormData = {
			status: "draft",
			signed_on: formData.signed_on,
			type: formData.type,
			data: formData.data,
			user: user.id,
		};
			...
			if (response.status === 200 || response.status === 201) {
				...
				if (status == "submitted") {
					...
					if (submitResponse.status === 200) {
						alert("Form submitted and approval steps initialized!");
					} else {
						alert("Form created but submission failed.");
					}
				} else {
	...
	return (
		...
		>
			<Paper sx={{ width: "100%", padding: 10 }}>
				<Typography p>
					Student Center North, N203, Houston, TX 77204-3024{" "}
					<CallIcon />
					Phone: (713) 743-5065 <EmailIcon /> Email:
					isssohlp@central.uh.edu <PublicIcon />{" "}
					<Link href={"http://uh.edu/oisss"} underline={"always"}>
						{" "}
						http://uh.edu/oisss{" "}
					</Link>{" "}
				</Typography>
				...
				<Typography p>
					...
					while studying in the U.S. Undergraduate and
					post-baccalaureate students are expected to complete a
					minimum of 12 hours of course work during the fall and
					spring semesters. Classes during the summer are optional
					unless it is the first semester at UH; then an F-1 student
					has to complete 6 hours (i.e. full-time for summer). ). The
					following form must be completed before dropping below
					full-time hours after start of classes.
				</Typography>
				<Typography p>
					<b>Note:</b> Dropping below full course load may involve the
					loss of resident tuition based on a scholarship, grant, or
					on-campus employment.
				</Typography>
				<Typography p sx={{ textDecoration: "underline" }}>
					Please complete the form below by selecting one of the
					options:
				</Typography>
				<form onSubmit={(e) => e.preventDefault()}>
					<FormControl fullWidth margin={"normal"}>
						<Typography variant={"h5"}>
							{" "}
							1. ACADEMIC DIFFICULTY <u>(FIRST SEMESTER ONLY)</u>
						</Typography>
						<Box sx={{ pl: 2 }}>
							<Typography p>
								RCL for valid academic difficulties is allowed
								once and only in the first semester when
								starting a new degree program. A minimum of 6hrs
								will still have to completed. This option cannot
								be used or submitted prior to ORD.
							</Typography>
							...
							<Typography
								...
								sx={{ pt: 2 }}
							>
								ICLP CERTIFYING SIGNATURE BY PROFESSOR
							</Typography>
							<Typography sx={{ fontStyle: "italic" }} p>
								I recommend that this student be allowed to drop
								the following course(s) due to improper course
								level placement as defined above.
							</Typography>
							<Box
								component="form"
								sx={{
							...
							>
								...
								<Button
									variant="text"
									onClick={() =>
										handleOpenSignature("professor1")
									}
									fullWidth
									disabled={isViewOnly}
									sx={{
										backgroundImage: formData.data
											.classes[0].signature
							...
							>
								...
								<Button
									variant="text"
									onClick={() =>
										handleOpenSignature("professor2")
									}
									fullWidth
									disabled={isViewOnly}
									sx={{
							...
							>
								...
								<Button
									variant="text"
									onClick={() =>
										handleOpenSignature("professor3")
									}
									fullWidth
									disabled={isViewOnly}
									sx={{
							...
							<FormControlLabel
								...
								control={
									<Checkbox
										...
										checked={
											formData.data.medicalLetterAttached
										}
										onChange={handleCheckboxChange}
										disabled={isViewOnly}
									/>
						...
						<Box sx={{ pl: 2 }}>
							<FormControlLabel
								...
								label={
									<>
										{" "}
										This is my final semester and I only
										need
										<TextField
											...
											size="small"
											value={
												formData.data.finalSemesterHours
											}
											onChange={handleInputChange}
											sx={{
												width: "150px",
											...
											disabled={isViewOnly}
										/>
										hours of course work to complete my
										degree. I understand that if I am
										granted a reduced course load and fail
										to complete my degree as planned, I may
										be in violation of my legal status and
										will need to apply for reinstatement.
										(If you need only one course to finish
										your program of study, it cannot be
										taken through online/distance
										education).
									</>
								}
							/>
							...
							<FormControlLabel
								...
								control={
									<Checkbox
										...
										checked={
											formData.data.concurrentlyEnrolled
										}
										onChange={handleCheckboxChange}
										disabled={isViewOnly}
									/>
								...
								label={
									<>
										I am taking courses at another
										college/University and want to drop a
										course at UH. I will still have 12 hours
										of enrollment between both schools.
										After the drop, I will have
										<TextField
											name="hoursUH"
											type="number"
											label="hours at UH"
										...
										<TextField
											...
											size="small"
											value={
												formData.data.hoursOtherSchool
											}
											onChange={handleInputChange}
											sx={{
												width: "150px",
										...
										<TextField
											...
											disabled={isViewOnly}
										/>
										. Attach proof of concurrent enrollment.
										Academic advisor signature is not
										required for this option, only ISSSO
										counselor.
									</>
								}
							/>
						...
						<Box sx={{ border: 1, mt: 2 }}>
							...
							<Typography p>
								You must submit a copy of this form to Office of
								the University Registrar (located in the Welcome
								Center) if you are requesting the drop after the
								1st day of the semester. The approval signature
								from your Academic Advisor and ISSSO are
								required to drop a course. You may still be
								responsible for the tuition and fee charges to
								the dropped course(s) after passing the
								deadline.
								<Box
									component="form"
									sx={{
						...
						<Box sx={{ mt: 2 }}>
							...
							<Typography
								...
								gutterBottom
							>
								APPROVAL SIGNATURE FROM ISSSO (if course drop is
								required)
							</Typography>
							<Box
								component="form"
							...
							>
								...
								<Button
									variant="text"
									onClick={() => handleOpenSignature("isso")}
									fullWidth
									disabled={isViewOnly}
									sx={{
										backgroundImage: formData.data
			...
			<Dialog open={open} onClose={handleClose} maxWidth="md">
				<DialogTitle>
					{activeSignatureId === "student"
						? "Student Signature"
						: activeSignatureId === "advisor"
							? "Advisor Signature"
							: activeSignatureId === "isso"
								? "ISSO Signature"
								: activeSignatureId?.startsWith("professor")
									? "Professor Signature"
									: "Signature"}
				</DialogTitle>
				<DialogContent>

...
def get_pending_approvals_for_user(user):
    ...
    including those delegated to them
    """
    # Find all approver roles for this user
    try:
    ...
    # Collect all forms that this user can approve
    pending_forms = Form.objects.filter(status='submitted')
    forms_to_approve = []
    
    for form in pending_forms:
        # Skip if the form has no approval steps
        if not hasattr(form, 'approval_steps'):
            continue
            
        # Check if user is a direct approver for any step
        direct_approver = form.approval_steps.filter(
            approver=user,
            is_completed=False
        ).exists()
        
        # Check if user is delegated for any approver in the approval steps
        delegated_approver = False
        for delegation in delegations:
            delegated_approver = form.approval_steps.filter(
        ...
        # Check if user is an org-level approver
        org_approver = is_approver and approver_scope == 'org'
        
        # Check if user is a unit-level approver for this form's unit
        unit_approver = False
        if is_approver and approver_scope == 'unit' and approver_unit and form.user.unit:
            # Check if form's unit is in approver's jurisdiction
            if form.user.unit == approver_unit or form.user.unit.parent == approver_unit:
                unit_approver = True
            ...
            for step_data in data['steps']:
                ...
                if 'approver_unit' in step_data and step_data['approver_unit']:
                    try:
                        ...
                        raise ValidationError(f"Unit with ID {step_data['approver_unit']} does not exist")
